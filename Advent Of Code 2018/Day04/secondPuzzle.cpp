...
pair<int, int> GetSleepiestMinute(int guardID)
{
    ...
    return {sleepiestMinute, slept[sleepiestMinute]};
}
...
int main()
{
    ...
    for (map<int, vector<Timestamp>>::iterator i = guardTimestamps.begin(); i != guardTimestamps.end(); i++)
    {
        pair<int, int> currSleepiestMinute = GetSleepiestMinute((*i).first);
        if (currSleepiestMinute.second > mostSleep)
        {
            guardID = (*i).first;
            sleepiestMinute = currSleepiestMinute.first;
            mostSleep = currSleepiestMinute.second;
        }
    }

    cout << sleepiestMinute * guardID << endl;
